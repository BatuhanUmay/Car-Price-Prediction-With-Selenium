...
frame = { 'Marka': df_marka, 'Seri': df_seri , 'Model': df_model}
newDf = pd.DataFrame(frame)

last_seri = newDf.drop_duplicates('Seri')
last_model = newDf.drop_duplicates('Model')
...
df_copy = df.copy()
df_copy = df_copy.drop_duplicates("Seri")
df_copy = df_copy.drop_duplicates("Model")
...
# series = st.selectbox("Seri:", last_seri.loc[last_seri['Marka'] == brand]['Seri'].tolist())
# model = st.selectbox("Model:", last_model.loc[last_model['Seri'] == series]['Model'].tolist())
series = st.selectbox("Seri:", df_copy.loc[df_copy['Marka'] == brand]['Seri'].tolist())
model = st.selectbox("Model:", df_copy.loc[df_copy['Seri'] == series]['Model'].tolist())
...
fuelEfficiency = st.number_input("Ortalama yakıt tüketimi:")
fuelTank = st.number_input("Yakıt deposu:", format="%d", value=0)
replacedParts = st.text_input("Boya-değişen:")
